...
function Plan(){
    ...
    return(
        <>
            <div className="text-black sm:w-[75%] w-[80%] max-sm:flex max-sm:flex-col grid grid-flow-col grid-cols-3 gap-4 py-5">
                <div className="text-sm">
                    <h1 className="text-2xl">價格方案</h1>
                    <p>選擇適合你的方案，開始使用我們的服務！</p>
                </div>
                {Price && Price.map((product: PriceModel) => (
                    <div key={product.id} className="flex flex-col items-start gap-5 border-2 p-4 rounded-xl">
                        <h2>{product.name}</h2>
                        <p>價格: <span className="font-bold text-xl">{product.price}</span>元/月</p>
